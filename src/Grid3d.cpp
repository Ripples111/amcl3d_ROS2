...
Grid3d::Grid3d(): rclcpp::Node("amcl3d_node_Grid3d")
{
    //Grid3d_ = std::make_shared<amcl3d::Grid3d>(this);
}
...
bool Grid3d::open(const std::string& map_path, const double sensor_dev)
{
  try
  {
    ...
    RCLCPP_INFO(this->get_logger(),"[%s] Octomap loaded", this->get_name());
    ...
    RCLCPP_INFO(this->get_logger(),"[%s]"
             ...
             "\n      Z: %lf to %lf"
             "\n      Res: %lf",
             this->get_name(), pc_info_->octo_min_x, pc_info_->octo_max_x, pc_info_->octo_min_y,
             pc_info_->octo_max_y, pc_info_->octo_min_z, pc_info_->octo_max_z, pc_info_->octo_resol);
  }
  catch (std::exception& e)
  {
    RCLCPP_ERROR(this->get_logger(),"[%s] %s", this->get_name(), e.what());
    return false;
  }
  ...
  RCLCPP_INFO(this->get_logger(),"[%s] Computing 3D occupancy grid. This will take some time...", this->get_name());
  try
  {
  ...
  RCLCPP_INFO(this->get_logger(),"[%s] Computing 3D occupancy grid done!", this->get_name());
...
bool Grid3d::buildGridSliceMsg(const double z, nav_msgs::msg::OccupancyGrid& msg) const
{
...
bool Grid3d::buildMapPointCloudMsg(sensor_msgs::msg::PointCloud2& msg) const
{
...
bool Grid3d::saveGrid(const std::string& grid_path)
{
  ...
  if (!pf)
  {
    RCLCPP_ERROR(this->get_logger(),"[%s] Error opening file %s for writing", this->get_name(), grid_path.c_str());
    return false;
  }
  ...
  RCLCPP_INFO(this->get_logger(),"[%s] Grid map successfully saved on %s", this->get_name(), grid_path.c_str());
...
bool Grid3d::loadGrid(const std::string& grid_path, const double sensor_dev)
{
  ...
  if (!pf)
  {
    RCLCPP_WARN(this->get_logger(),"[%s] Error opening file %s for reading", this->get_name(), grid_path.c_str());
    return false;
  }
  ...
  {
    RCLCPP_WARN(this->get_logger(),"[%s] Loaded sensorDev is different", this->get_name());
    return false;
  }
  ...
  RCLCPP_INFO(this->get_logger(),"[%s] Grid map successfully loaded from %s", this->get_name(), grid_path.c_str());
